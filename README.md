...
The system is intentionally data-centric, permission-aware, and deterministic, favoring clear workflows and long-term maintainability.
...
Local Configuration
•	Runtime configuration is read from environment variables or an ignored `.env` file.
•	Use `.env.example` as the non-secret template for local setup.
•	Required database values:
o	MORO_DB_HOST
o	MORO_DB_NAME
o	MORO_DB_USER
o	MORO_DB_PASS
•	MORO_APP_ENV=production disables development-only relaxed portal role switching.
•	MORO_RELAXED_PORTAL_ROLES=1 may be used locally while role/verification workflows are still being built.
